...
**Owner:** Supplier Service workstream  
**Project:** Friend on Campus (FoC), CS3219 AY2627S1, Group 5

This document incorporates the Supplier workstream's D2 planning decisions. The User Service contract in §6 is a concrete proposal to build against; its owner must confirm it before integrated testing. D1 backlog changes are called out explicitly.

## 1. Scope

Supplier Service owns the catalogue of campus stores, facilities, and pickup locations. Its private database and independently callable HTTP APIs support authenticated member browsing and administrator management. A responsive frontend uses those APIs.

### 1.1 D2 outcome

The near-complete D2 demonstration includes: a justified database and schema; working list, name/location search, building/category filters, sorting, pagination, and detail; database-backed create, update, archive, and restore APIs callable with the UI stopped; authenticated requests using real User Service identity and role checks; and a responsive live frontend for members and administrators. Seed the catalogue from the repository CSV and add useful locations for a rich listing.
...
| Database | PostgreSQL, private to Supplier Service | Selected |
...
| Removal | Archive, retain record, allow administrator restoration | D1 backlog revision |
...
| `building` | varchar(120), nonblank; canonical spelling | Location filter/display |
| `floor` | varchar(20), nullable | Floor or named level |
| `location_description` | varchar(300), nonblank | Directions within the building |
| `latitude`, `longitude` | decimals, nullable together; valid ranges | Optional map coordinates |
| `opens_at`, `closes_at` | times, nullable together | Typical hours; overnight closing permitted |
| `image_path` | varchar(500), nullable | URL/path for a served image |
| `status` | ACTIVE or ARCHIVED, not null | Catalogue visibility |
...
**`supplier_categories`** has `supplier_id` (UUID FK to `suppliers.id`) and controlled `category` (`FOOD`, `COFFEE`, `SHOPPING`, `PRINTING`, `FACILITY`, `LANDMARK`, or agreed extension), with composite primary key (`supplier_id`, `category`). Application validation requires at least one category and stores category changes in the same transaction as supplier edits. Multiple categories accommodate the seed's `Food/Coffee` value.

Index the primary key, status/building, and category membership. Sort names with ID as a stable tiebreaker. Use ordinary case-insensitive substring search over the small catalogue; measure before adding specialist indexes. Do not create cross-service foreign keys.
...
- Map the CSV's **Location Description** to `location_description`. There is no separate general description field in this revised design.
- Normalize labels such as `Com 2`/`Com2`; split `Food/Coffee` into two categories.
- Parse `0900hrs` as `09:00`. `11:00`–`02:00` crosses midnight. Label these *typical hours*; weekdays are not specified.
...
### 4.3 D1 description revision

D1 SS-F1.1.2 and SS-F2.1.1 explicitly require a general `description`. The team should amend those backlog items and acceptance criteria: in this design, `name` and category identify the supplier, while `locationDescription` tells users where to find it. Do not claim to have implemented the original general-description requirement. A distinct description can be restored later if needed.
...
This revises D1 SS-F2.2.3, which blocked deletion while an active errand existed. The future product rule is that archiving prevents new errand selection while existing errands keep the pickup details they need. Order Service behaviour is **not implemented or tested in D2**. Demonstrate catalogue visibility, retention, and restoration without claiming order integration.
...
**List query:** `q` searches `name` and `locationDescription` case-insensitively; `building` matches a normalized building; `category` is controlled text; `status=ARCHIVED` is admin-only; `page` is zero-based (default 0); `size` defaults to 12 (maximum 100); `sort` allowlist: `name,asc`, `name,desc`, `updatedAt,desc`. Trim `q`; blank means no search. Reject invalid values with 400. Filter before pagination; break ties by ID. Admins can request ACTIVE or ARCHIVED explicitly, not an unspecified all-status view.
...
**Create/update body:** editable fields only: `name`, nonempty `categories`, `building`, optional `floor`, required `locationDescription`, optional paired coordinates, optional paired hours, optional `imagePath`. Trim text; reject missing/blank fields, invalid categories, out-of-range coordinates, one-sided coordinates/hours, oversized values, and unsupported image URLs/paths. Hours are *typical*; agree on an explicit representation for a 24-hour location instead of inferring 24/7 from `00:00`–`23:59`.
...
There is no alternate version-in-request-body or 409 concurrency contract.
...
- **Performance:** D1's p95 under two seconds for list/detail is measured under a stated workload, including synchronous identity validation. Optimize indexes after measuring.
...
3. Log in as ADMINISTRATOR: create, edit, archive, inspect ARCHIVED, and restore; inspect API/database effects.
...
**Confirm with the team:** User Service cookie/login/proxy, validation endpoint and service credential, timeout and idle-refresh rules, CSRF issuance/validation; D1 revisions for description and archive; canonical buildings/categories, image paths, and hours representation. Publish the shared auth contract in the repository. NestJS/Express, PostgreSQL/Drizzle, Vite React, and ETag/If-Match are selected for the Supplier workstream.
