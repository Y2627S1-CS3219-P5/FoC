 ...
 * detection, lifecycle no-ops, SQL state, and edit-preserving repeat seed.
...
async function run() {
  ...
  const update = await supplierRequest(
    `/api/v1/suppliers/${createdId}`,
    adminToken,
    {
      method: "PUT",
  ...
  const replayedUpdate = await supplierRequest(
    `/api/v1/suppliers/${createdId}`,
    adminToken,
    {
      method: "PUT",
  ...
  assertError(replayedUpdate, 412, "SUPPLIER_VERSION_CONFLICT");
  ...
  console.info(
    "PASS: real auth, mutation authorization, validation/preconditions, " +
      ...
      "archive/restore lifecycle no-ops, SQL retention, repeat-seed " +
